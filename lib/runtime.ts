...
import { RequestScript, ResponseScript } from "./scripting";
 ...
 */
export function preRequestRuntime(functionBody: string, req: RequestScript): TestResult[] {
  try {
    ...
    return sandboxedFunction(expect, req);
  } catch (error) {
...
export function postResponseRuntime(functionBody: string, res: ResponseScript): TestResult[] {
  try {
    // Create a sandboxed execution environment
    const sandboxedFunction = new Function(
      "expect", "res",
    ...
    // Execute the sandboxed function and return results
    return sandboxedFunction(expect, res);
  } catch (error) {
    console.log("Script Execution Error");
    return [];
  }
